    ...
    try {
        ...
        if (response) {
            ...
            setProfile(prevProfile => {
                ...
                return { 
                    ...prevProfile, 
                    roles: response.roles || roleToUpdate.value 
                };
            });

            // Update selected role
            const matchingRoleOption = roleOptions.find(
                option => option.value === (response.roles || roleToUpdate.value)
            );
    ...
    confirmDialog({
      ...
      accept: async () => {
        try {
          ...
          if (response.ok) throw new Error('Mosque removed successfull!');
            ...
            <div className="flex-1 flex flex-column p-4">
              ...
              <div className="border-top-1 border-300 pt-3 mt-auto">
                ...
                <div className="flex align-items-center">
                  ...
                  <span>dhuhr: {mosque.prayerTimes?.dhuhr || "N/A"}</span>
                  <span>asr: {mosque.prayerTimes?.asr || "N/A"}</span>
                  <span>magrib: {mosque.prayerTimes?.maghrib || "N/A"}</span>
                  <span>isha: {mosque.prayerTimes?.isha || "N/A"}</span>
                </div>
              </div>
            </div>
          </div>
    ...
    return (
      <div className="flex justify-content-center align-items-center min-h-screen">
        <Card title="Error" className="w-full md:w-6">
          <p>You are not Authenticated. Login first <a className='list-style-none' href="/Components/Auth/login">Login</a> </p>
